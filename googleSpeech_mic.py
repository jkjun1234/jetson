...
import Jetson.GPIO as GPIO      #toogle 스위치
import time

btn_pin = 18  # board pin 18 BCM 24
...
def CommandProc(stt):
    ...
    for cmdList in cmdLists:
        ...
        if str(cmd) == cmdList[0]:
            ############# insert code ############## 
            if cmdList[0] == "나이는":
               #os.system("python myprj.py")     ## start detect front
               f = open("getLabelsPotions.txt", "r")     ##open Labels file
               data = f.read()
               data.strip()

               if data == '0':
                   tts.play("not detected")
               else: 
                   tts.play(data)
...
def listen_print_loop(responses, mic):
    ...
    """
    #GPIO.add_event_detect(btn_pin, GPIO.FALLING, callback=sw_interrupt, bouncetime=10)
    print("@@@@@@@@loop1\n")
    num_chars_printed = 0
...
def main():
    ...
    #GPIO.setup(btn_pin, GPIO.IN)    #gpio set
    ...
    with MicrophoneStream(RATE, CHUNK) as stream:
        ...
        tts.play("안녕하십니까? 스위치를 킨 후 명령을 내려주세요")
